...
import FullNavbar from '../home/FullNavbar';
import { RxCross1 } from "react-icons/rx";
import { NavBarContext } from '../../src/context/Navcontext';
// import { useGSAP } from '@gsap/react';

function Navbar({svgColor='white'}) {
    const btnAnimation = useRef(null);
    const [navOpen, setNavOpen]=useContext(NavBarContext)
  
  return (
    <>
    <div className='flex fixed w-full z-[9999] justify-between text-white font-[font500] top-0 left-0 '>
        <Link to='/' className='text-[5vw] p-[1vw]'><svg  xmlns="http://www.w3.org/2000/svg" fill={svgColor} width="118" height="49" viewBox="0 0 103 44">
                    ...
                    <div 
                    onMouseEnter={()=>{
                        btnAnimation.current.style.height='100%'
                    }}
                    onMouseLeave={()=>{
                        btnAnimation.current.style.height='0%'
                    }}
                    className='bg-black h-16 relative w-[18vw] '
                    >
                    <Link ref={btnAnimation} className=' bg-[#d3fd50] absolute top-0 h-0 w-full transition-all text-[4vw] pl-[8vw]'
                    >{navOpen 
                    ? <RxCross1 onClick={() => setNavOpen(false)} /> 
                    : <TbMenu onClick={() => setNavOpen(true)} /> }</Link>
